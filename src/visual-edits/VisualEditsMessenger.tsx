...
export const CHANNEL = "SEMANTIC_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "semantic_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "semantic_focused_element" as const;

// Deduplicate helper for high-frequency traffic (HIT / FOCUS_MOVED / SCROLL)
// -----------------------------------------------------------------------------
let _semanticLastMsg = "";
const postMessageDedup = (data: any) => {
  try {
    const key = JSON.stringify(data);
    if (key === _semanticLastMsg) return; // identical – drop
    _semanticLastMsg = key;
  } catch {
...
// Helper to parse data-semantic-id to extract file path, line, and column
const parseSemanticId = (
  semanticId: string
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data-semantic-protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data-semantic-protected");
  ...
  const handleStyleChange = (
    ...
    const existingStyles = appliedStylesRef.current.get(semanticId) || {};
    appliedStylesRef.current.set(semanticId, { ...existingStyles, ...styles });
  ...
  const handleStyleBlur = (element: HTMLElement) => {
    ...
    const appliedStyles = appliedStylesRef.current.get(semanticId);
  ...
  const flushImageSrcChange = () => {
    ...
    const semanticId = imgElement.getAttribute("data-semantic-id");
    if (!semanticId) return;
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      if (e.data?.type === "SEMANTIC_STYLE_UPDATE") {
        const { elementId, styles } = e.data;

        // Find ALL elements with the same orchids ID
        const allMatchingElements = document.querySelectorAll(
          `[data-semantic-id="${elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      } else if (e.data?.type === "SEMANTIC_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data-semantic-id
        const semanticId = element.getAttribute("data-semantic-id");
        if (semanticId) {
          const parsed = parseSemanticId(semanticId);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data-semantic-id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const hitId = hit.getAttribute("data-semantic-id");
        ...
        const tagName =
          hit.getAttribute("data-semantic-name") || hit.tagName.toLowerCase();

        // Update hover boxes immediately for instant feedback
        // Find ALL elements with the same orchids ID
        const allMatchingElements = document.querySelectorAll(
          `[data-semantic-id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        allMatchingElements.forEach((element) => {
          ...
          const elementId = element.getAttribute("data-semantic-id");
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      const target = e.target as HTMLElement;
      const hit = target.closest<HTMLElement>("[data-semantic-id]");
      ...
      if (hit) {
        ...
        // Clicked on empty space or element without data-semantic-id
    ...
    function onMsg(e: MessageEvent<ParentToChild>) {
      ...
      if (e.data.msg === "CLEAR_INLINE_STYLES" && "elementId" in e.data) {
        // Find ALL elements with the same orchids ID
        const allMatchingElements = document.querySelectorAll(
          `[data-semantic-id="${e.data.elementId}"]`
        ) as NodeListOf<HTMLElement>;
      ...
      if (e.data.msg === "SHOW_ELEMENT_HOVER" && "elementId" in e.data) {
        ...
        if (allMatchingElements.length > 0) {
          ...
          allMatchingElements.forEach((element) => {
            ...
            if (!tagName) {
              tagName =
                element.getAttribute("data-semantic-name") ||
