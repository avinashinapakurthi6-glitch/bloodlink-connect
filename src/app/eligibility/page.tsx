    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-red-950/20 to-slate-950 pl-72 p-8">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">Eligibility Checker</h1>
          <p className="text-slate-400">Check if you&apos;re eligible to donate blood based on health criteria</p>
        </div>

        {result && (
          <div className={`mb-8 p-6 rounded-2xl border ${result.eligible ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'}`}>
            <div className="flex items-center gap-4 mb-4">
              <span className="text-4xl">{result.eligible ? '✅' : '❌'}</span>
              <div>
                <h2 className={`text-2xl font-bold ${result.eligible ? 'text-green-400' : 'text-red-400'}`}>
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900/80 backdrop-blur rounded-2xl p-8 border border-slate-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl bg-slate-800 border border-slate-700 text-white focus:border-red-500 outline-none"
                placeholder="e.g., 25"
              />
            </div>
            <div>
          ...
          <h3 className="text-lg font-semibold text-white mb-6">Health Metrics</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Hemoglobin (g/dL)</label>
              <input
                type="number"
                step="0.1"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Blood Pressure (Systolic)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Blood Pressure (Diastolic)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-2">Pulse Rate (bpm)</label>
              <input
                type="number"
          ...
          <h3 className="text-lg font-semibold text-white mb-6">Health Conditions</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            {[
              ...
              <label key={item.key} className="flex items-center gap-3 p-4 rounded-xl bg-slate-800/50 border border-slate-700 cursor-pointer hover:border-slate-600 transition-colors">
                <input
                  type="checkbox"
                  checked={formData[item.key as keyof typeof formData] as boolean}
                  onChange={() => handleCheckboxChange(item.key)}
                  className="w-5 h-5 rounded border-slate-600 text-red-500 focus:ring-red-500 bg-slate-700"
                />
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold text-lg shadow-lg hover:shadow-green-500/30 transition-all disabled:opacity-50"
          >
