    ...
    async function fetchAlerts() {
      ...
      if (!error && data) {
        const alertData = data.map((item) => ({
          ...
          severity: item.units <= 5 ? "critical" : item.units <= 15 ? "low" : "normal",
        }));
        setAlerts(alertData.filter((a) => a.severity !== "normal"));
      }
      setLoading(false);
    }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-orange-100">
                  ...
                  {alerts.filter((a) => a.severity === "low").length} low stock warnings
                </p>
              </div>
            </div>
          </div>
